...
// Problem 1
// The event director needs a list of all names in Alphbetical order.
let firstName = [];
runners.forEach(runner => {
	if (!firstName.includes(runner["first_name"])) {
...
// Problem 2
// The event director needs a list of everyone with small sized shirts.
let runnersSmallSizeShirt = [];

const smallSized = object => {
	return object.shirt_size === "S";
};
runnersSmallSizeShirt = runners.filter(smallSized);

console.log(runnersSmallSizeShirt);

// Problem 3
// The event director needs a list of last names and ID for the runners shirts.
let shirtNumbers = [];

runners.forEach(items => shirtNumbers.push(`${items.last_name} ${items.id}`));
